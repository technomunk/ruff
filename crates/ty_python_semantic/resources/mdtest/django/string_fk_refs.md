...
Book.objects.filter(author__account_id=1)
Book.objects.filter(author__account_pk=1)
Book.objects.filter(author__account__pk=1)
Book.objects.filter(author__account__id=1)
Book.objects.filter(author__account__name="team")
```

## ForeignKey with dotted app string reference

Passing a dotted Django model reference (`ForeignKey("myapp.Author", ...)`) resolves through the
app's `models` module.
...
class Book(Model):
    author = ForeignKey("myapp.Author", on_delete=None)
```
