...
at runtime. `JSONField` stores arbitrary JSON data and resolves to `Unknown`.
...
`/.venv/<path-to-site-packages>/django/core/files/__init__.py`:

```py
```

`/.venv/<path-to-site-packages>/django/core/files/base.py`:
...
class JSONField(Generic[_ST, _GT]):
    def __init__(self, *, null: bool = False, blank: bool = False, default=None): ...
```

`/.venv/<path-to-site-packages>/django/db/models/fields/files.py`:
...
class Media(Model):
    ...
    photo = ImageField()
    config = JSONField()
    typed_config = JSONField[dict[str, object], dict[str, object]]()

m = Media()
reveal_type(m.data)  # revealed: bytes
reveal_type(m.optional_data)  # revealed: bytes | None
reveal_type(m.upload)  # revealed: FieldFile
reveal_type(m.upload.name)  # revealed: str
reveal_type(m.photo)  # revealed: ImageFieldFile
reveal_type(m.photo.width)  # revealed: int
reveal_type(m.config)  # revealed: Unknown
reveal_type(m.typed_config)  # revealed: Unknown

Media.objects.filter(upload="")
Media.objects.filter(photo="")
Media.objects.filter(typed_config={"theme": "dark"})
Media.objects.create(upload="uploads/report.pdf")
Media(upload="uploads/report.pdf", photo="uploads/photo.png")
Media.objects.create(upload=ContentFile(b"report"))
Media(upload=ContentFile(b"report"), photo=ContentFile(b"photo"))
m.upload = ContentFile(b"report")
m.photo = ContentFile(b"photo")
```
...
## Model constructor validates field keyword arguments

Django models accept field names in their constructor and reject statically known unknown fields.
...
article = Article(title="Django", count=1)
Article(unexpected=True)  # error: [unknown-argument]
reveal_type(article)  # revealed: Article
```

## Model constructor accepts writable properties

Django model initialization assigns remaining keyword arguments with `setattr`, so writable
properties are accepted even when they are not concrete model fields.
...
class Token(Model):
    legacy_access = CharField(max_length=255)

    @property
    def access(self) -> str:
        return self.legacy_access

    @access.setter
    def access(self, value: str) -> None:
        self.legacy_access = value

token = Token(access="secret")
Token(access=object())  # error: [invalid-argument-type]
Token(missing="secret")  # error: [unknown-argument]
reveal_type(token.access)  # revealed: str
```
