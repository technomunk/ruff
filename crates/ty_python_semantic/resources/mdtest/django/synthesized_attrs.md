...
## Model exception classes are available

Django adds `DoesNotExist`, `NotUpdated`, and `MultipleObjectsReturned` exception classes to each
model class.
...
reveal_type(Article.DoesNotExist)  # revealed: type[Exception]
reveal_type(Article.NotUpdated)  # revealed: type[Exception]
reveal_type(Article.MultipleObjectsReturned)  # revealed: type[Exception]
...
## Choice fields synthesize display method

Django adds `get_<field>_display()` for fields declared with `choices=`.
...
```py
class CharField:
    def __init__(self, *, max_length: int = 255, choices=None): ...
```
...
class Article(Model):
    status = CharField(max_length=20, choices=(("draft", "Draft"), ("live", "Live")))
    title = CharField(max_length=100)

a = Article()
reveal_type(a.get_status_display())  # revealed: str
a.get_title_display()  # error: [unresolved-attribute]
```

## Date fields synthesize next and previous methods

Django adds `get_next_by_<field>()` and `get_previous_by_<field>()` for `DateField` and
`DateTimeField`.
...
a = Article()
reveal_type(a.get_next_by_published_on())  # revealed: Article
reveal_type(a.get_previous_by_updated_at())  # revealed: Article
a.get_next_by_title()  # error: [unresolved-attribute]
```
